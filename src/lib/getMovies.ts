...
export const getNowPlayingMovies = async () => {
  const url = new URL(`${process.env.TMBD_URL}/3/movie/now_playing`);
  const data = await fetcher(url);
  return data?.results;
};

export const getPopularMovies = async () => {
  const url = new URL(`${process.env.TMBD_URL}/3/movie/popular`);
  const data = await fetcher(url);
  return data?.results;
};

export const getUpcomingMovies = async () => {
  const url = new URL(`${process.env.TMBD_URL}/3/movie/upcoming`);
  const data = await fetcher(url);
  return data?.results;
};
...
export const getMovieVideos = async (id?: string) => {
  const url = new URL(`${process.env.TMBD_URL}/3/movie/${id}/videos`);
  const data = await fetcher(url);
  return data?.results;
};

export const getMovieDetails = async (id?: string) => {
  const url = new URL(`${process.env.TMBD_URL}/3/movie/${id}`);
  const data = await fetcher(url);
  return data;
};
