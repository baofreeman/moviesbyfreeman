...
const MovieDetailPage = async ({ params: { id } }: Props) => {
  ...
  const videos = movies.map((movie: any) => ({
    ...
    published_at: movie.punlished_at,
    site: movie.site,
    size: movie.size,
    type: movie.type,
  }));
      ...
      <div className="px-10">
        <div className="py-10 flex flex-col lg:flex-row items-center gap-5">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-2">
            ...
            <p className="text-sm laeding-6 tracking-wide mt-2">
              {details?.overview}
            </p>
            <p className="text-gray-200 text-sm">
              IMBD{" "}
              <span className="text-wthite font-medium">
            ...
            <p className="text-gray-200 text-sm">
              ...
              {details?.genres.map((genre: any) => (
                <span key={genre?.id} className="text-white font-medium mr-1">
                  {genre?.name}
                </span>
              ))}
            </p>
